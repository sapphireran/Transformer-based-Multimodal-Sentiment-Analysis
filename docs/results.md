...
The CSVs below were produced by the personal training runs that
originally lived next to this code. They are checked in under
`model/results/` and `model/mosi_test/`. Duplicate copies at
`model/*.csv` match the `results/` directory.

This page transcribes the tables and only states what they contain.
Interpretation lives in [`research-notes.md`](research-notes.md).

Metric definitions: [`metrics.md`](metrics.md).
Remember Acc-7 / Acc-5 use **uniform bins on [-3, 3]**, and Acc-2 / F1
**drop zeros** and use *binary* (not weighted) F1.
...
Best baseline on every reported column: **TransformerLate**.
...
Full GMTM beats TransformerLate on MAE (0.5640 vs 0.5846), Acc-2, Corr,
and F1. Text-only GMTM is already better than every BERT *baseline* on
MAE / Corr / F1.
...
Best baseline: **LowRankTensorFusion**. TransformerLate is no longer
first once the language stream is 300-D GloVe instead of BERT.
...
Full GMTM wins MAE / Acc-7 / Acc-5 / Corr against the other GloVe GMTM
rows, but **does not beat LRTF's MAE** (0.6251 vs 0.6174). The
text+audio row is a clear regression versus text-only.

## MOSI · BERT · transfer from MOSEI checkpoints

Source: `model/mosi_test/mosi_bert_results.csv`

Pooled MOSI (train+valid+test concatenated). Not the official test split.
...
## MOSI · BERT · GMTM ablation (pooled)
...
Audio-only Corr is **negative**. Full GMTM is worse than text+visual
on this pooled MOSI BERT transfer.
...
Here GMTM is the clear winner among the logged GloVe MOSI rows.

## MOSI · GloVe · GMTM ablation (pooled)
...
Same pattern as GloVe MOSEI: text+audio is harmful; the full triple
recovers and is the best GMTM row.

## How to reprint the tables

```bash
python3 examples/summarize_logged_results.py
```

The script reads the CSVs above, prints Markdown-ish aligned columns,
and flags the best value per numeric column (min for MAE, max for the
rest).
